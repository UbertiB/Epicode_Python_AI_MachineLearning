...
#una riga 2 colonna
root.grid_rowconfigure(0, weight=1)  #RIGA
root.grid_columnconfigure(0, weight=0) #COLONNA1 controlli: larghezza “naturale”
root.grid_columnconfigure(1, weight=1)  #COLONNA2 output: si espande

controls = ttk.Frame(root, padding=10)
controls.grid(row=0, column=0, sticky="nsw")

output = ttk.Frame(root, padding=10)
...
grid_linee = ttk.Frame(controls)
grid_linee.grid(row=r, column=0, sticky="ew", pady=(5, 10))
r += 1
...
for i, linea in enumerate(linee):
    ...
    ttk.Checkbutton(
        ...
        text=linea,
        variable=var,
        command=aggiorna_output
    ).grid(row=rr, column=cc, sticky="w", padx=6, pady=3)

# Pulsanti tutte/nessuna
