...
Matplotlib dispone di widgets incorporati che si possono aggiungere direttamente ai grafici:
...
per modificare i parametri di un modello matematico.

I PULSANTI permettono di eseguire operazioni discrete, come resettare un grafico o cambiare il 
dataset di un grafico

I CHECKBOX gestiscono la visibilità di curve multiple o layer di informazioni. possono essere combinati con slider
Contentono di attivare/disattivare gli elementi che stiamo visualizzando.

Rendono l'esperienza interativa
...
funzioni di linking tra widget e grafici che aggiornano dinamicamente le figure

Matplotlib può essere integrato in GUI desktop con Tkinter (semplice e leggero)e 
con PyQt e PySide, più potente e layout complessi.
Questa integrazione permette di costruiere strumenti completi di analisi dei dati,
dove i grafici sono interattivi e non statici e input utenti e calcoli numerici 
convivono in un unica applicazione.

Matplotlib suporta interativà leggera con i suoi widget
Con Ipywidgets diventa potente quando si lavora con notebook, grafici iterativi molto
più potenti
...
ax_amp = plt.axes([0.25, 0.1, 0.65, 0.03]) #definisco un contenitore di assi in cui inseriro' gli slider
